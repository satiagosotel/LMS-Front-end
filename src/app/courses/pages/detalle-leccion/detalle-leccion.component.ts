...
import { MatCardContent, MatCard, MatCardActions, MatCardHeader, MatCardTitle, MatCardSubtitle, MatCardAvatar } from "@angular/material/card";
...
export class DetalleLeccionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cursosService
      .listar(`/api/lessons/${this.idLeccion}`)
      .subscribe((response: response) => {
        if (response.status == 'SUCCESS') {
          this.leccion = response.data;
          this.videoUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
            this.leccion.youtubeUrl,
          );
        }
      });
  }
